...
class Flux2BaseDataLoader(
    ...
    def _preparation_modules(self, config: TrainConfig, model: Flux2Model):
        ...
        # Edit mode: encode custom_conditioning_image (loaded from -condlabel.png by mixin)
        rescale_conditioning_image = RescaleImageChannels(
            image_in_name='custom_conditioning_image', image_out_name='custom_conditioning_image',
            in_range_min=0, in_range_max=1, out_range_min=-1, out_range_max=1
        )
        encode_conditioning_image = EncodeVAE(
            in_name='custom_conditioning_image', out_name='latent_conditioning_image_distribution',
            vae=model.vae, autocast_contexts=[model.autocast_context], dtype=model.train_dtype.torch_dtype()
        )
        ...
        if model.is_dev():
            tokenize_prompt = Tokenize(
                ...
                tokenizer=model.tokenizer, max_token_length=config.text_encoder_sequence_length,
            ...
            encode_prompt = EncodeMistralText(
                ...
                text_encoder=model.text_encoder, autocast_contexts=[model.autocast_context],
            ...
            encode_prompt = EncodeQwenText(
                tokens_name='tokens', tokens_attention_mask_in_name='tokens_mask',
                hidden_state_out_name='text_encoder_hidden_state', tokens_attention_mask_out_name='tokens_mask',
                text_encoder=model.text_encoder, hidden_state_output_index=QWEN3_HIDDEN_STATES_LAYERS,
        ...
        # Edit mode: encode the custom_conditioning_image loaded from -condlabel.png files
    ...
    def _cache_modules(self, config: TrainConfig, model: Flux2Model):
        image_split_names = ['latent_image', 'original_resolution', 'crop_offset']

        if config.masked_training or config.model_type.has_mask_input():
            image_split_names.append('latent_mask')

        # Edit mode: cache conditioning image latents
        if config.custom_conditioning_image:
    ...
    def _output_modules(self, config: TrainConfig, model: Flux2Model):
        output_names = [
        ...
        # Edit mode: include conditioning image latents in output
        if config.custom_conditioning_image:
        ...
        return self._output_modules_from_out_names(
            ...
            use_conditioning_image=config.custom_conditioning_image,
            vae=model.vae,
