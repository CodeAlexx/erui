"""
QwenImageEditDataLoader - Data loader for Qwen-Image-Edit training.

Handles paired image loading (control image -> target image) for image editing training.
Based on SimpleTuner's approach to Qwen edit training.
"""
...
class QwenImageEditDataLoader(
    BaseDataLoader,
    DataLoaderText2ImageMixin,
):
    """
    Data loader for Qwen-Image-Edit training.

    Key differences from QwenBaseDataLoader:
    - Loads paired images: control (source) image and target image
    - Encodes control image to latents for transformer input
    - Uses -condlabel suffix for control images (same as FLUX Fill pattern)
    """
    ...
    def _preparation_modules(self, config: TrainConfig, model: QwenImageEditModel):
        ...
        encode_image = EncodeVAE(
            ...
            vae=model.vae, autocast_contexts=[model.autocast_context],
        ...
        # Control/conditioning image encoding (for image editing)
        ...
        encode_conditioning_image = EncodeVAE(
            in_name='conditioning_image', out_name='latent_conditioning_image_distribution',
            vae=model.vae, autocast_contexts=[model.autocast_context],
            dtype=model.train_dtype.torch_dtype()
        )
        conditioning_image_sample = SampleVAEDistribution(
            in_name='latent_conditioning_image_distribution', out_name='latent_conditioning_image',
            mode='mean'
        )
        ...
        # Always encode conditioning image for image editing
    ...
    def _cache_modules(self, config: TrainConfig, model: QwenImageEditModel):
        image_split_names = ['latent_image', 'latent_conditioning_image', 'original_resolution', 'crop_offset']
    ...
    def _output_modules(self, config: TrainConfig, model: QwenImageEditModel):
        output_names = [
            'image_path', 'latent_image', 'latent_conditioning_image',
            'prompt',
            'tokens',
            'tokens_mask',
    ...
    def _debug_modules(self, config: TrainConfig, model: QwenImageEditModel):
        ...
        decode_conditioning_image = DecodeVAE(
            in_name='latent_conditioning_image', out_name='decoded_conditioning_image',
            vae=model.vae, autocast_contexts=[model.autocast_context],
            dtype=model.train_dtype.torch_dtype()
        )
        ...
        save_image = SaveImage(
            image_in_name='decoded_image', original_path_in_name='image_path',
            path=debug_dir, in_range_min=-1, in_range_max=1, before_save_fun=before_save_fun
        )
        save_conditioning_image = SaveImage(
            image_in_name='decoded_conditioning_image', original_path_in_name='image_path',
            path=debug_dir + "/conditioning", in_range_min=-1, in_range_max=1,
    ...
    def create_dataset(
        ...
        # Enable conditioning image loading for image edit
        # This uses -condlabel suffix pattern (same as FLUX Fill)
        config = copy.copy(config)
        config.custom_conditioning_image = True
