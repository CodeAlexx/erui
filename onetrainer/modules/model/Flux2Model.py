...
class Flux2Model(BaseModel):
    ...
    @staticmethod
    def prepare_conditioning_ids(latents: torch.Tensor, scale: float = 10.0, ref_idx: int = 0) -> torch.Tensor:
        """
        Prepare position IDs for conditioning/reference images in edit mode.
        Uses time-offset encoding where T = scale + scale * ref_idx to distinguish
        reference images from target images (which have T=0).

        Args:
            latents: The conditioning latent tensor [batch, channels, height, width]
            scale: The conditioning scale (default 10.0, from config)
            ref_idx: Index of the reference image (0 for first reference)

        Returns:
            Position IDs tensor for the conditioning latents
        """
        batch_size, _, height, width = latents.shape

        # Time offset: scale + scale * ref_idx (e.g., 10 for first ref, 20 for second)
        t_value = scale + scale * ref_idx
        t = torch.full((1,), t_value, device=latents.device, dtype=torch.long)
        h = torch.arange(height, device=latents.device)
        w = torch.arange(width, device=latents.device)
        l_ = torch.arange(1, device=latents.device)
