...
class ZImageModel(BaseModel):
    ...
    def text_encoder_to(self, device: torch.device): #TODO share more code between models
        if self.text_encoder is not None:
            ...
            # always move the text encoder. The conductor handles the layers that are offloaded,
            # but the rest of the model (embeddings, final layers) must be moved to the device 
