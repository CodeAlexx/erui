...
class ChromaSampler(BaseModelSampler):
    ...
    def __sample_base(
        ...
        with self.model.autocast_context:
            ...
            # Split embeddings for memory-efficient CFG (run positive and negative separately)
            prompt_embedding = combined_prompt_embedding[0:1]  # positive
            negative_embedding = combined_prompt_embedding[1:2]  # negative
            prompt_mask = text_attention_mask[0:1]
            negative_mask = text_attention_mask[1:2]
            
            image_seq_len = latent_image.shape[1]
            single_image_attention_mask = torch.full((1, image_seq_len), True, dtype=torch.bool, device=text_attention_mask.device)
            
            for i, timestep in enumerate(tqdm(timesteps, desc="sampling")):
                expanded_timestep = timestep.expand(1)
                
                # Run positive prompt
                positive_attention_mask = torch.cat([prompt_mask, single_image_attention_mask], dim=1)
                noise_pred_positive = transformer(
                    hidden_states=latent_image.to(dtype=self.model.train_dtype.torch_dtype()),
                ...
                # Run negative prompt (only if cfg_scale != 1.0)
                if cfg_scale != 1.0:
                    negative_attention_mask = torch.cat([negative_mask, single_image_attention_mask], dim=1)
                    noise_pred_negative = transformer(
                        hidden_states=latent_image.to(dtype=self.model.train_dtype.torch_dtype()),
                        timestep=expanded_timestep / 1000,
                        encoder_hidden_states=negative_embedding.to(dtype=self.model.train_dtype.torch_dtype()),
                        txt_ids=text_ids.to(dtype=self.model.train_dtype.torch_dtype()),
                        img_ids=image_ids.to(dtype=self.model.train_dtype.torch_dtype()),
