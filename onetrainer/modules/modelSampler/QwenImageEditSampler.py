"""
QwenImageEditSampler - Generates samples for Qwen-Image-Edit during training.

Unlike the text-to-image QwenSampler, this takes a control image as input
...
class QwenImageEditSampler(BaseModelSampler):
    def __init__(
            self,
    ...
    def load_control_image(self, path: str, height: int, width: int) -> torch.Tensor:
    ...
    def __sample_base(
        ...
        with self.model.autocast_context:
            ...
            # Prepare control image latents (for image editing)
            control_latents = None
            if control_image_path and Path(control_image_path).exists():
                self.model.vae_to(self.train_device)
                control_image = self.load_control_image(control_image_path, height, width)
                control_latents = self.model.encode_control_image(control_image, self.train_device)
                self.model.vae_to(self.temp_device)
                torch_gc()

            # Prepare latent image (start from noise or noise + control)
    ...
    def sample(
        ...
        sampler_output = self.__sample_base(
            ...
            control_image_path=getattr(sample_config, 'control_image_path', None),
