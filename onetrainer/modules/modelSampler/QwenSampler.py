    ...
    def __sample_base(
        ...
        with self.model.autocast_context:
            ...
            for i, timestep in enumerate(tqdm(timesteps, desc="sampling")):
                ...
                if cfg_scale > 1.0:
                    # Sequential CFG: process positive and negative separately to save VRAM
                    # This uses half the memory at the cost of 2x transformer calls
                    noise_pred_positive = transformer(
                        ...
                        encoder_hidden_states=combined_prompt_embedding[0:1].to(dtype=self.model.train_dtype.torch_dtype()),
                        encoder_hidden_states_mask=text_attention_mask[0:1],
                        txt_seq_lens=[txt_seq_lens[0]],
                        img_shapes=[img_shapes[0]],
                        attention_kwargs = {
                            "attention_mask": attention_mask_2d[0:1],
                        },
                        return_dict=True
                    ).sample
                    ...
                    noise_pred_negative = transformer(
                        hidden_states=latent_image.to(dtype=self.model.train_dtype.torch_dtype()),
                        timestep=expanded_timestep / 1000,
                        encoder_hidden_states=combined_prompt_embedding[1:2].to(dtype=self.model.train_dtype.torch_dtype()),
                        encoder_hidden_states_mask=text_attention_mask[1:2],
                        txt_seq_lens=[txt_seq_lens[1]],
                        img_shapes=[img_shapes[1]],
                        attention_kwargs = {
                            "attention_mask": attention_mask_2d[1:2],
                        },
                        return_dict=True
                    ).sample
