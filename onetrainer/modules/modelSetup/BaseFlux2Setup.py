...
#                apply_circular_padding_to_conv2d(model.transformer_lora)
    ...
    def predict(
        ...
        with model.autocast_context:
            ...
            target_seq_len = packed_latent_input.shape[1]  # Store for edit mode extraction

            # Edit mode: process conditioning image and concatenate
            if config.edit_mode and 'latent_conditioning_image' in batch:
                # Process conditioning image through same pipeline
                conditioning_latent = model.patchify_latents(batch['latent_conditioning_image'].float())
                scaled_conditioning_latent = model.scale_latents(conditioning_latent)
                packed_conditioning = model.pack_latents(scaled_conditioning_latent)

                # Create conditioning position IDs with time offset
                conditioning_ids = model.prepare_conditioning_ids(
                    conditioning_latent,
                    scale=config.conditioning_scale,
                    ref_idx=0  # First (and typically only) reference image
                )

                # Concatenate target and conditioning along sequence dimension
                # [batch, target_seq + cond_seq, channels]
                packed_latent_input = torch.cat([packed_latent_input, packed_conditioning], dim=1)
                image_ids = torch.cat([image_ids, conditioning_ids], dim=1)
            ...
            # Edit mode: extract only target tokens, drop conditioning tokens
            if config.edit_mode and 'latent_conditioning_image' in batch:
                packed_predicted_flow = packed_predicted_flow[:, :target_seq_len, :]
