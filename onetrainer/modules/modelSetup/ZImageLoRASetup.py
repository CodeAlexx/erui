...
class ZImageLoRASetup(
    ...
    def setup_model(
        ...
        # Debug: verify PEFT adapter parameters
        adapter_params = list(model.transformer_lora.parameters())
        print(f"[DEBUG] PEFT adapter ({config.peft_type.value}) params count: {len(adapter_params)}")
        if adapter_params:
            print(f"[DEBUG] First param requires_grad: {adapter_params[0].requires_grad}, dtype: {adapter_params[0].dtype}, device: {adapter_params[0].device}")
