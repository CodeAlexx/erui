...
class LycorisModuleWrapper:
    """Wrapper for the official lycoris-lora library.

    Provides the same interface as LoRAModuleWrapper for seamless integration
    with OneTrainer's existing architecture.
    """

    # Map PeftType to lycoris algorithm names
    ALGO_MAP = {
        PeftType.LOKR: 'lokr',
    ...
    def __init__(
        ...
        self._lycoris_prefix = "lycoris"  # Default, will be updated

        if orig_module is None:
            return

        algo = self.ALGO_MAP.get(config.peft_type)
        if algo is None:
            raise ValueError(f"Unsupported LyCORIS PEFT type: {config.peft_type}")

        # Configure layer targeting via preset
        if module_filter:
            # Use custom filter patterns
            LycorisNetwork.TARGET_REPLACE_NAME = module_filter
            LycorisNetwork.USE_FNMATCH = not config.layer_filter_regex
        else:
            # Use default full preset
            LycorisNetwork.TARGET_REPLACE_MODULE = ["Linear", "Conv2d"]
            LycorisNetwork.TARGET_REPLACE_NAME = []

        # Build kwargs for create_lycoris based on algorithm
        kwargs = {
            'multiplier': 1.0,
            'linear_dim': config.lora_rank,
            'linear_alpha': config.lora_alpha,
            'algo': algo,
            'dropout': config.dropout_probability,
            'use_tucker': config.lycoris_use_tucker,
            'use_scalar': config.lycoris_use_scalar,
            'train_norm': config.lycoris_train_norm,
            'rescaled': config.lycoris_rescaled,
            'bypass_mode': config.lycoris_bypass_mode,
            'dora_wd': config.lycoris_dora_wd,
        }

        # Algorithm-specific parameters
        if algo == 'lokr':
            kwargs['factor'] = config.lycoris_factor
            kwargs['decompose_both'] = config.lycoris_decompose_both
            kwargs['full_matrix'] = config.lycoris_full_matrix
        elif algo == 'boft':
            kwargs['block_size'] = config.lycoris_block_size

        self.lycoris_net = create_lycoris(orig_module, **kwargs)
        self._lycoris_prefix = LycorisNetwork.LORA_PREFIX

        print(f"Created LyCORIS network with {len(self.lycoris_net.loras)} {algo} modules")

    def requires_grad_(self, requires_grad: bool):
        if self.lycoris_net is not None:
            self.lycoris_net.requires_grad_(requires_grad)

    def parameters(self) -> list[Parameter]:
        if self.lycoris_net is None:
            return []
        # lycoris_net.parameters() returns empty - must iterate through loras
        params = []
        for lora in self.lycoris_net.loras:
            params.extend(lora.parameters())
        return params

    def to(self, device: torch.device = None, dtype: torch.dtype = None) -> 'LycorisModuleWrapper':
        if self.lycoris_net is not None:
            self.lycoris_net.to(device=device, dtype=dtype)
        return self

    def load_state_dict(self, state_dict: dict[str, Tensor], strict: bool = True):
        """Load state dict into LyCORIS network."""
        if self.lycoris_net is None:
            return

        # LyCORIS uses its own naming convention with "lycoris_" prefix
        # Just pass the state dict directly - lycoris handles the key matching
        if state_dict:
            self.lycoris_net.load_state_dict(state_dict, strict=False)

    def state_dict(self) -> dict:
        """Return state dict from LyCORIS network."""
        if self.lycoris_net is None:
            return {}
        return self.lycoris_net.state_dict()

    def modules(self) -> list[nn.Module]:
        """Return list of all LyCORIS modules."""
        if self.lycoris_net is None:
            return []
        return list(self.lycoris_net.modules())

    def hook_to_module(self):
        """Apply LyCORIS adapters to the target module."""
        if self.lycoris_net is not None and not self._is_applied:
            self.lycoris_net.apply_to()
            self._is_applied = True

    def remove_hook_from_module(self):
        """Remove LyCORIS adapters from the target module."""
        if self.lycoris_net is not None and self._is_applied:
            self.lycoris_net.restore()
            self._is_applied = False

    def apply_to_module(self):
        """Merge LyCORIS weights into the target module."""
        if self.lycoris_net is not None:
            self.lycoris_net.merge_to(weight=1.0)

    def extract_from_module(self, base_module: nn.Module):
        """Not supported for LyCORIS - use standard training instead."""
        raise NotImplementedError("extract_from_module is not supported for LyCORIS adapters")

    def prune(self):
        """No-op for LyCORIS - no dummy modules to prune."""
        pass

    def set_dropout(self, dropout_probability: float):
        """Set dropout probability for all LyCORIS modules."""
        if dropout_probability < 0 or dropout_probability > 1:
            raise ValueError("Dropout probability must be in [0, 1]")
        if self.lycoris_net is not None:
            for lora in self.lycoris_net.loras:
                if hasattr(lora, 'dropout'):
                    lora.dropout = dropout_probability

    @property
    def lora_modules(self) -> dict:
        """Compatibility property for code that expects lora_modules dict."""
        if self.lycoris_net is None:
            return {}
        return {lora.lora_name: lora for lora in self.lycoris_net.loras}


def create_peft_wrapper(
        orig_module: nn.Module | None,
        prefix: str,
        config: TrainConfig,
        module_filter: list[str] = None,
) -> 'LoRAModuleWrapper | LycorisModuleWrapper | MultiAdapterWrapper':
    """Factory function to create the appropriate PEFT wrapper.

    Returns MultiAdapterWrapper if config.adapters has multiple entries,
    LycorisModuleWrapper for LyCORIS PEFT types (LoKr, LoCon, IA3, DyLoRA, GLoRA, BOFT),
    and LoRAModuleWrapper for traditional types (LoRA, LoHa, OFT_2).
    """
    # Check for multi-adapter mode
    if hasattr(config, 'adapters') and config.adapters and len(config.adapters) > 1:
        return MultiAdapterWrapper(orig_module, prefix, config, module_filter)

    if config.peft_type.is_lycoris():
        return LycorisModuleWrapper(orig_module, prefix, config, module_filter)
    else:
        return LoRAModuleWrapper(orig_module, prefix, config, module_filter)


class MultiAdapterWrapper:
    """Wrapper for stacking multiple PEFT adapters on the same module.

    Supports LyCORIS multi-adapter feature where adapters are applied in sequence,
    each potentially targeting different layers with different algorithms.
    """
    ...
    def __init__(
        ...
        self.orig_module = orig_module
        self.prefix = prefix
        self.adapters: list[LoRAModuleWrapper | LycorisModuleWrapper] = []
        self.adapter_configs = config.adapters if hasattr(config, 'adapters') else []

        if orig_module is None:
            return

        # Create each adapter from the config
        for i, adapter_cfg in enumerate(self.adapter_configs):
            if not adapter_cfg.get('enabled', True):
                continue

            # Create a modified config for this specific adapter
            adapter_config = self._create_adapter_config(config, adapter_cfg)

            # Determine layer filter for this adapter
            target_layers = adapter_cfg.get('target_layers', '')
            adapter_filter = target_layers.split(',') if target_layers else module_filter

            # Create the appropriate wrapper
            adapter_prefix = f"{prefix}_adapter{i}"
            if adapter_config.peft_type.is_lycoris():
                wrapper = LycorisModuleWrapper(orig_module, adapter_prefix, adapter_config, adapter_filter)
            else:
                wrapper = LoRAModuleWrapper(orig_module, adapter_prefix, adapter_config, adapter_filter)

            self.adapters.append(wrapper)
            print(f"Created adapter {i+1}: {adapter_cfg.get('name', f'Adapter {i+1}')} "
                  f"({adapter_cfg.get('peft_type', 'LORA')}) r={adapter_cfg.get('rank', 16)}")

        print(f"MultiAdapterWrapper: Created {len(self.adapters)} adapters for {prefix}")

    def _create_adapter_config(self, base_config: TrainConfig, adapter_cfg: dict) -> TrainConfig:
        """Create a config object for a single adapter from the adapter dict."""
        from copy import deepcopy

        # Create a shallow copy to avoid modifying the original
        config = deepcopy(base_config)

        # Override with adapter-specific settings
        peft_type_str = adapter_cfg.get('peft_type', 'LORA')
        config.peft_type = PeftType[peft_type_str] if isinstance(peft_type_str, str) else peft_type_str
        config.lora_rank = adapter_cfg.get('rank', base_config.lora_rank)
        config.lora_alpha = adapter_cfg.get('alpha', base_config.lora_alpha)
        config.dropout_probability = adapter_cfg.get('dropout', base_config.dropout_probability)
        config.lora_decompose = adapter_cfg.get('weight_decompose', base_config.lora_decompose)

        # LyCORIS-specific
        config.lycoris_use_tucker = adapter_cfg.get('use_tucker', base_config.lycoris_use_tucker)
        config.lycoris_use_scalar = adapter_cfg.get('use_scalar', base_config.lycoris_use_scalar)
        config.lycoris_train_norm = adapter_cfg.get('train_norm', base_config.lycoris_train_norm)
        config.lycoris_factor = adapter_cfg.get('factor', base_config.lycoris_factor)
        config.lycoris_decompose_both = adapter_cfg.get('decompose_both', base_config.lycoris_decompose_both)
        config.lycoris_full_matrix = adapter_cfg.get('full_matrix', base_config.lycoris_full_matrix)
        config.lycoris_block_size = adapter_cfg.get('block_size', base_config.lycoris_block_size)

        # OFT-specific
        config.oft_block_size = adapter_cfg.get('block_size', base_config.oft_block_size)

        return config

    def requires_grad_(self, requires_grad: bool):
        for adapter in self.adapters:
            adapter.requires_grad_(requires_grad)
    ...
    def to(self, device: torch.device = None, dtype: torch.dtype = None) -> 'MultiAdapterWrapper':
        for adapter in self.adapters:
            adapter.to(device, dtype)
        return self

    def load_state_dict(self, state_dict: dict[str, Tensor], strict: bool = True):
        """Load state dict - distributes to each adapter based on prefix."""
        for adapter in self.adapters:
            adapter.load_state_dict(state_dict, strict=False)

    def state_dict(self) -> dict:
        """Return combined state dict from all adapters."""
        state_dict = {}
        for adapter in self.adapters:
            state_dict.update(adapter.state_dict())
        return state_dict

    def modules(self) -> list[nn.Module]:
        """Return list of all modules from all adapters."""
        modules = []
        for adapter in self.adapters:
            modules.extend(adapter.modules())
        return modules

    def hook_to_module(self):
        """Apply all adapters in order (stacking)."""
        for adapter in self.adapters:
            adapter.hook_to_module()

    def remove_hook_from_module(self):
        """Remove all adapter hooks in reverse order."""
        for adapter in reversed(self.adapters):
            adapter.remove_hook_from_module()

    def apply_to_module(self):
        """Merge all adapter weights into the target module."""
        for adapter in self.adapters:
            adapter.apply_to_module()

    def extract_from_module(self, base_module: nn.Module):
        """Not supported for multi-adapter."""
        raise NotImplementedError("extract_from_module is not supported for MultiAdapterWrapper")

    def prune(self):
        """Prune dummy modules from all adapters."""
        for adapter in self.adapters:
            if hasattr(adapter, 'prune'):
                adapter.prune()

    def set_dropout(self, dropout_probability: float):
        """Set dropout for all adapters."""
        for adapter in self.adapters:
            adapter.set_dropout(dropout_probability)

    @property
    def lora_modules(self) -> dict:
        """Combined lora_modules from all adapters."""
        combined = {}
        for adapter in self.adapters:
            if hasattr(adapter, 'lora_modules'):
                combined.update(adapter.lora_modules)
        return combined
