...
class OffloadingWindow(ctk.CTkToplevel):
    ...
    def __content_frame(self, master):
        ...
        # Musubi block swap for training
        components.label(frame, 4, 0, "Block Swap (Training)",
                         tooltip="Musubi block swap for training: streams transformer blocks between CPU and GPU during forward/backward passes. 0 = auto (uses ~half blocks when checkpointing on), or specify exact count. Reduces VRAM but increases training time. Applies to K5, ZImage, Qwen.")
        components.entry(frame, 4, 1, self.ui_state, "musubi_blocks_to_swap")
