...
class TrainCallbacks:
    def __init__(
            self,
            on_update_train_progress: Callable[[TrainProgress, int, int, float | None, float | None], None] = lambda _, __, ___, **kwargs: None,
    ...
    def on_update_train_progress(
        ...
        if self.__on_update_train_progress:
            with contextlib.suppress(Exception):
                self.__on_update_train_progress(train_progress, max_step, max_epoch, loss=loss, smooth_loss=smooth_loss)
