...
class TrainCommands:
    ...
    def update_epochs(self, new_epochs: int):
        """Update target epochs during training."""
        self.__update_epochs_value = new_epochs
        if self.__on_command:
            self.__on_command(self)

    def get_and_reset_update_epochs_command(self) -> int | None:
        """Returns new epochs value if set, None otherwise."""
        value = getattr(self, '_TrainCommands__update_epochs_value', None)
        self.__update_epochs_value = None
        return value
    ...
    def merge(self, other):
        ...
        if other.get_and_reset_save_command():
            self.save()
        new_epochs = other.get_and_reset_update_epochs_command()
        if new_epochs is not None:
            self.update_epochs(new_epochs)
